...
export default function Login() {
  const signInWithGoogle = async () => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: "https://prep-pilot-blush.vercel.app/auth/callback",
      },
    });
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 py-12 md:px-12 lg:px-16 xl:px-24 space-y-8">
          ...
          <h2 className="text-3xl font-semibold text-gray-900 leading-tight">
            Transform Your <span className="text-blue-600">Hiring Process</span>{" "}
            with AI
          </h2>
          ...
          <div className="mt-6 flex items-center space-x-4">
            <div className="flex -space-x-2">
              {[1, 2, 3, 4].map((item) => (
                <div
                  key={item}
                  className="h-10 w-10 rounded-full bg-blue-100 border-2 border-white"
                />
              ))}
            </div>
            <div>
